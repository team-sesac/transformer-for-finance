...
class DoubleAxisDataProcessor:
    def __init__(self, file_paths, usecols):
        self.file_paths = file_paths
        self.dfs = self.import_and_list_dataframes(file_paths, usecols)
    ...
    @staticmethod
    def import_and_list_dataframes(file_paths, usecols=None):
        """
        여러 개의 데이터프레임을 읽어오고 리스트로 반환하는 함수

        Parameters:
        - file_paths (list): 각 데이터프레임의 파일 경로가 담긴 리스트
        - usecols (list): 주가 데이터에 사용할 feature 리스트
        ...
        return [pd.read_csv(file_path, usecols=usecols) for file_path in file_paths]
