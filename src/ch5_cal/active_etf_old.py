...
def calculate_active_etf_returns(target_day=252):
    ...
    active_etf_stock_pred_price_df = _load_csv('scaled_close_preds.csv')[-target_day:]
    ...
    # 기하평균
    adjusted_returns = active_etf_stock_price_df_increment_cumsum.fillna(0) + 1
    geometric_cumulative_returns = adjusted_returns.cumprod() - 1
    portfolio_geometric_cumulative_return = (geometric_cumulative_returns.prod(axis=1) ** (1/len(geometric_cumulative_returns.columns))) - 1

    _save_file(geometric_cumulative_returns, 'active_etf_geometric_cumulative_returns.csv')
    _save_file(portfolio_geometric_cumulative_return, 'active_etf_portfolio_geometric_cumulative_return.csv')
    return geometric_cumulative_returns.tail(), portfolio_geometric_cumulative_return.tail()
    
    # 산술평균? 종목별 누적 수익률 cumsum()
